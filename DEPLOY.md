...
Статический сайт находится в папке `site/`. Основной способ деплоя — **Vercel** (рекомендуется). GitHub Pages больше не поддерживается.
...
- **Основной способ деплоя** — Vercel (GitHub Pages больше не поддерживается).
- Для продакшена подключите свой домен в Vercel.
